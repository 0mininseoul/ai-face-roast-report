...
const sections = [
  ...
  },
  {
    ...
    body: [
      "서비스는 Gemini API를 통한 이미지 및 텍스트 분석, Supabase를 통한 데이터베이스 및 파일 저장, Vercel 또는 이에 준하는 플랫폼을 통한 배포와 서버리스 실행, Kakao 공유 기능 또는 브라우저 내 공유 기능을 사용할 수 있습니다. 이러한 외부 제공자는 각자의 인프라 위치와 정책에 따라 데이터를 국외에서 처리하거나 저장할 수 있습니다.",
      "외부 서비스로 전송되는 정보의 범위는 기능 제공에 필요한 수준으로 제한하려고 하나, Gemini API에 전송되는 분석 요청에는 얼굴 이미지 또는 얼굴 관련 메트릭이 포함될 수 있고, Supabase에는 결과 조회를 위해 필요한 이미지, 결과 JSON, 생성 시각 및 만료 시각이 저장될 수 있습니다. 사용자는 이러한 외부 처리 구조를 원하지 않는 경우 서비스를 이용하지 않아야 합니다.",
    ],
  },
  {
    title: "6. 보유 기간",
