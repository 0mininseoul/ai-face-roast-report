...
export function AnalyzePage({ locale = DEFAULT_LOCALE }: { locale?: Locale }) {
  return (
    <Suspense fallback={<AnalyzeFallback locale={locale} />}>
      <AnalyzeClient locale={locale} />
    </Suspense>
  );
}

function AnalyzeFallback({ locale }: { locale: Locale }) {
  ...
  return (
    <main className="grid h-screen place-items-center bg-black text-text-muted">
      <div className="flex items-center gap-3 text-sm font-semibold">
        ...
        {dictionary.analyze.preparing}
      </div>
    </main>
  );
}

function AnalyzeClient({ locale }: { locale: Locale }) {
  const router = useRouter();
  const dictionary = getDictionary(locale);
  const livenessWarningText = dictionary.analyze.livenessWarning;
  ...
  const { state, start: startAnalysis } = useAnalysisStream({ locale, onEvent: handleAnalysisEvent });
  ...
  useEffect(() => {
    ...
    if (invalidGender || invalidTone) router.replace(`/${locale}`);
  }, [gender, locale, router, toneParam]);
  ...
  useEffect(() => {
    ...
    setFaceWarning(dictionary.analyze.engineRestarting);
    logEvent("face_landmarker_runtime_error", { ...faceDetectionDiagnostics(), message: faceLandmarkerError }, null, "error");
  }, [dictionary.analyze.engineRestarting, faceDetectionDiagnostics, faceLandmarkerError, logEvent]);
    ...
    async (samples: Landmark[][], trigger: AnalysisTrigger) => {
      ...
      try {
        ...
        if (!isLivenessAcceptable(liveness)) {
          ...
          setFaceWarning(livenessWarningText);
          return false;
        }
        ...
        if (!video || video.readyState < 2 || !faceVisibleRef.current || landmarkLossCountRef.current !== landmarkLossCountAtSettleStart) {
          ...
          sampleCollectionStartedAtRef.current = Date.now();
          setSampleCount(0);
          setFaceWarning(dictionary.analyze.facePosition);
        ...
        void startAnalysis({
          ...
          const message = caught instanceof Error ? caught.message : dictionary.analyze.requestFailed;
          logEvent("analysis_client_request_failed", { message }, null, "error");
          setClientError(message);
        });
      } catch (caught) {
        const message = caught instanceof Error ? caught.message : dictionary.analyze.metricFailed;
        logEvent("analysis_client_prepare_failed", { message }, null, "error");
        setClientError(dictionary.analyze.metricFailedLong);
      }

      return true;
    },
    [analysisTone, dictionary.analyze.facePosition, dictionary.analyze.metricFailed, dictionary.analyze.metricFailedLong, dictionary.analyze.requestFailed, gender, livenessWarningText, locale, logEvent, startAnalysis, status, videoRef],
  );
  ...
  useEffect(() => {
    if (!landmarks) {
      ...
      if (lastSeenAt && lostForMs < LANDMARK_LOSS_RESET_MS) {
        setFaceWarning(dictionary.analyze.trackingUnstable);
        return;
      }
    ...
    const livenessWarning = Date.now() < livenessWarningUntilRef.current ? livenessWarningText : null;
    ...
    setFaceWarning(faceCount > 1 ? dictionary.analyze.oneFaceOnly : livenessWarning);
  ...
  }, [beginAnalysis, detectionSource, dictionary.analyze.facePosition, dictionary.analyze.oneFaceOnly, dictionary.analyze.trackingUnstable, faceDetectionDiagnostics, faceLandmarkerDelegate, landmarks, livenessWarningText, logEvent, result]);
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      ...
      if (samples.length === 0 && elapsedMs >= ZERO_SAMPLE_CPU_FALLBACK_MS && faceLandmarkerDelegate === "GPU" && !zeroSampleFallbackLoggedRef.current) {
        ...
        setFaceWarning(dictionary.analyze.engineReboot);
      ...
      if (samples.length === 0 && elapsedMs >= ZERO_SAMPLE_FAIL_MS && faceLandmarkerDelegate === "CPU" && !zeroSampleFailureLoggedRef.current) {
        ...
        setFaceWarning(dictionary.analyze.detectionSlow);
        return;
      }
  ...
  }, [beginAnalysis, clientError, dictionary.analyze.detectionSlow, dictionary.analyze.engineReboot, faceDetectionDiagnostics, faceLandmarkerDelegate, logEvent, status, uploadDiagnosticFrame]);
  ...
  const progress = useMemo(
    ...
    [isLoading, locale, sampleCount, state.isComplete, state.jobStatus, state.pollCount, state.raw.length, state.reportId, status],
  );
  const cards = useMemo(() => buildCards(state.sections, locale), [locale, state.sections]);
  const loadingCards = useMemo(() => buildCards(null, locale), [locale]);
  ...
  return (
    <main ref={rootRef} className="relative h-screen overflow-hidden bg-black">
      ...
      <div className="fixed left-7 top-6 z-20 flex items-center gap-2 rounded-lg border border-border bg-black/45 px-3 py-2.5 text-xs font-bold uppercase tracking-[0.14em] text-text-muted backdrop-blur">
        ...
        <span>{dictionary.brand.liveBadge}</span>
      </div>
      ...
      {!isFatal && !state.sections && (
        <AnalysisStatusPanel percent={progress.percent} label={state.statusMessage ?? progress.label} text={dictionary.analyze.statusText} />
      )}
          ...
          return (
            <div key={item.card.key} ref={(node) => { cardRefs.current[item.card.key] = node; }}>
              <AnalysisCard
                ...
                text={item.isCompletedCard ? item.card.text : item.loadingCard?.text ?? dictionary.analyze.waiting}
              ...
              >
                {item.isCompletedCard && item.card.key === "scores" && state.sections ? <ScoreCardContent sections={state.sections} locale={locale} /> : null}
              </AnalysisCard>
            </div>
          );
        })}
      ...
      {conclusionItem && (
        <div ref={(node) => { cardRefs.current[conclusionItem.card.key] = node; }} className="fixed bottom-8 left-1/2 z-30 w-[min(980px,72vw)] -translate-x-1/2">
          <AnalysisCard
            ...
            text={conclusionItem.isCompletedCard ? conclusionItem.card.text : conclusionItem.loadingCard?.text ?? dictionary.analyze.finalizing}
            tone="verdict"
            ...
            action={
              conclusionItem.isCompletedCard && state.reportId ? (
                <Button className="w-full justify-center" icon={<ArrowRight className="h-4 w-4" />} onClick={() => router.replace(`/${locale}/result/${state.reportId}`)}>
                  {dictionary.analyze.resultCta}
                </Button>
              ) : null
            }
          />
      ...
      {isFatal && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/78">
          <div className="glass-panel max-w-lg rounded-2xl p-8 text-center">
            ...
            <h1 className="mt-5 text-2xl font-extrabold">{dictionary.analyze.fatalTitle}</h1>
            <p className="mt-3 text-sm leading-6 text-text-muted">{clientError ?? state.error ?? error ?? dictionary.analyze.cameraCheck}</p>
            <div className="mt-6 grid gap-2 sm:grid-cols-2">
              <Button icon={<RefreshCcw className="h-4 w-4" />} onClick={() => window.location.reload()}>
                {dictionary.analyze.retry}
              </Button>
              <Button variant="ghost" onClick={() => router.push(`/${locale}`)}>
                {dictionary.analyze.home}
              </Button>
            </div>
          </div>
        </div>
...
function ScoreCardContent({ sections, locale }: { sections: ReportSections; locale: Locale }) {
  const report = getDictionary(locale).report;
  const rows = [
    { label: report.likability, value: sections.scores.likability, comment: sections.scores.comments[0] },
    { label: report.trust, value: sections.scores.trust, comment: sections.scores.comments[1] },
    { label: report.symmetry, value: sections.scores.symmetry, comment: sections.scores.comments[2] },
    { label: report.balance, value: sections.scores.balance, comment: sections.scores.comments[3] },
    { label: report.attractiveness, value: sections.scores.attractiveness, comment: sections.scores.comments[4] },
  ];
...
function buildCards(sections: ReportSections | null, locale: Locale) {
  const dictionary = getDictionary(locale);
  const loading = dictionary.analyze.loadingCards;
  const report = dictionary.report;
  const cards = [
    { key: "geometry", title: "§1 FACIAL GEOMETRY", text: sections ? Object.values(sections.geometry).join(" ") : loading.geometry },
    { key: "forehead", title: "§2 FOREHEAD", text: sections ? `${sections.parts.forehead.metricsText} ${sections.parts.forehead.comment}` : loading.forehead },
    { key: "eyes", title: "§2 EYES", text: sections ? `${sections.parts.eyes.metricsText} ${sections.parts.eyes.comment}` : loading.eyes },
    { key: "nose", title: "§2 NOSE", text: sections ? `${sections.parts.nose.metricsText} ${sections.parts.nose.comment}` : loading.nose },
    { key: "mouth", title: "§2 MOUTH", text: sections ? `${sections.parts.mouth.metricsText} ${sections.parts.mouth.comment}` : loading.mouth },
    { key: "jaw", title: "§2 JAW", text: sections ? `${sections.parts.jaw.metricsText} ${sections.parts.jaw.comment}` : loading.jaw },
    { key: "skin", title: "§2 SKIN", text: sections ? `${sections.parts.skin.observation} ${sections.parts.skin.comment}` : loading.skin },
    {
      key: "scores",
      title: "§3 AESTHETIC INDEX",
      text: sections
        ? `${report.likability} ${sections.scores.likability}, ${report.trust} ${sections.scores.trust}, ${report.symmetry} ${sections.scores.symmetry}, ${report.balance} ${sections.scores.balance}, ${report.attractiveness} ${sections.scores.attractiveness}. ${sections.scores.comments.join(" ")}`
        : loading.scores,
    },
    { key: "impression", title: "§4 IMPRESSION", text: sections ? `${sections.impression.keywords.join(", ")}. ${sections.impression.physiognomy}` : loading.impression },
  ];
